...
object NNF {
  ...
  def nnf(expr: Expr): Expr = expr match {
    ...
    case maxCardinality(n, r, f) => maxCardinality(n, r, f)
    case Not(maxCardinality(n, r, f)) => minCardinality(n+1, r, f)
    
    case minCardinality(n, r, f) => 
      if (n >= 1) minCardinality(n, r, f)
      else if (n == 0) Top
      else {println("[ERROR] untreated: " + expr); null}
    case Not(minCardinality(n, r, f)) => 
      if (n >= 1) maxCardinality(n-1, r, f)
      else if (n == 0) Bottom
      else {println("[ERROR] untreated: " + expr); null}  
